...
void Game::DrawImage(const Sprite& sprite, const Vec2& position) {
	...
	int image_x_end = min(static_cast<int>(position.x) + static_cast<int>(image->width), world_x_pos_at_0_0 + window_width);
	int image_y_end = min(static_cast<int>(position.y) + static_cast<int>(image->height), world_y_pos_at_0_0 + window_height);
